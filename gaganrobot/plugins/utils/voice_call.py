...
from gaganrobot import gaganrobot, Message, pool, filters, get_collection, Config
from gaganrobot.utils import time_formatter
from gaganrobot.utils.exceptions import StopConversation
...
call = GroupCall(gaganrobot, play_on_repeat=False)
...
def check_enable_for_all(func):
    ...
    async def checker(msg: Message):
        if (
            (
                msg.from_user
                and msg.from_user.id == gaganrobot.id
            ) or CMDS_FOR_ALL
        ):
            await func(msg)
...
def check_cq_for_all(func):
    ...
    async def checker(_, c_q: CallbackQuery):
        if c_q.from_user.id == gaganrobot.id or CMDS_FOR_ALL:
            await func(c_q)
        else:
            await c_q.answer(
...
@gaganrobot.on_cmd("joinvc", about={
...
@gaganrobot.on_cmd("leavevc", about={
...
@gaganrobot.on_cmd("vcmode", about={
...
@gaganrobot.on_cmd("play", about={'header': "play or add songs to queue"},
...
@gaganrobot.on_cmd("helpvc", about={'header': "help for voice_call plugin"},
               trigger=Config.SUDO_TRIGGER, allow_private=False, check_client=True,
               filter_me=False, allow_bots=False)
@vc_chat
@check_enable_for_all
async def _help(msg: Message):
    ...
    commands = gaganrobot.manager.enabled_plugins["voice_call"].enabled_commands
    cmds = []
    raw_cmds = []
...
📘 <b>Doc:</b>  <code>GaganRobot Voice-Call Plugin</code>\n\n"""
...
@gaganrobot.on_cmd("forceplay", about={
...
@gaganrobot.on_cmd("current", about={
...
async def current(msg: Message):
    ...
    await reply_text(
        msg,
        BACK_BUTTON_TEXT,
        markup=default_markup() if gaganrobot.has_bot else None,
        to_reply=True
    )


@gaganrobot.on_cmd("queue", about={
...
@gaganrobot.on_cmd("volume", about={
    'header': "Set volume",
    'usage': "{tr}volume\n{tr}volume 69"},
    allow_private=False)
@vc_chat
...
@gaganrobot.on_cmd("resume", about={
    'header': "Resume Song.",
    'usage': "{tr}resume"},
    trigger=Config.SUDO_TRIGGER, check_client=True, allow_private=False,
    filter_me=False, allow_bots=False)
...
@gaganrobot.on_cmd("stopvc", about={
...
async def stop_music(msg: Message):
    ...
    await reply_text(msg, "`Stopped GaganRobot-Music.`", del_in=5)
...
if gaganrobot.has_bot:
    @gaganrobot.bot.on_callback_query(filters.regex("(skip|queue|back)"))
    @check_cq_for_all
    ...
    @gaganrobot.bot.on_callback_query(filters.regex(r"vol\((.+)\)"))
    @check_cq_for_all
    async def vol_callback(cq: CallbackQuery):
        ...
        if arg.isnumeric():
            ...
            try:
                async with gaganrobot.conversation(cq.message.chat.id, user_id=cq.from_user.id) as conv:
