...
from gaganrobot import gaganrobot, Message, Config, filters, get_collection, pool
from gaganrobot.utils import SafeDict, time_formatter
...
if gaganrobot.has_bot:
    gaganrobot_id = gaganrobot.id if gaganrobot.dual_mode else Config.OWNER_ID[0]
    bot = gaganrobot.bot
    ...
    async def start(_, msg: PyroMessage):
        ...
        if cmd and ' ' not in msg.text:
            commands = gaganrobot.manager.enabled_commands
    ...
    @bot.on_message(
        filters.user(gaganrobot_id) & filters.private & filters.command("settext"), group=1)
    ...
    @bot.on_message(filters.user(gaganrobot_id) & filters.private & filters.command("pmban"), group=1)
    async def pm_ban(_, msg: PyroMessage):
        ...
        elif replied and replied.from_user.id == gaganrobot_id:
    ...
    @bot.on_message(
        filters.user(gaganrobot_id) & filters.private & filters.command("pmunban"), group=1)
...
"""
            ...
            user_dict = await gaganrobot.get_user_dict(cq.from_user.id)
    ...
    async def handle_incoming_message(msg: PyroMessage):
        ...
        try:
            if msg.sticker:
                await bot.send_message(gaganrobot_id, f"{msg.from_user.mention} sent you a sticker.")
            m = await msg.forward(gaganrobot_id)
    ...
    async def handle_reply(msg: PyroMessage):
        ...
        elif replied.from_user.id == gaganrobot_id:
...
"""
    ...
    async def broadcast(msg: PyroMessage):
        ...
        IN_CONVO = True
        temp_msgs = []
        async with gaganrobot.bot.conversation(
