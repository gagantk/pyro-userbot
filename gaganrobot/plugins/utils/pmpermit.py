...
from gaganrobot import gaganrobot, filters, Message, Config, get_collection
from gaganrobot.utils import SafeDict
...
@gaganrobot.on_cmd("allow", about={
    'header': "allows someone to contact",
    'description': "Ones someone is allowed, "
                   "GaganRobot will not interfere or handle such private chats",
...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await gaganrobot.get_users(userid)).unblock()
...
@gaganrobot.on_cmd("listpm", about={
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await gaganrobot.get_users(user)).id
...
@gaganrobot.on_cmd("setpmmsg", about={
    'header': "Sets the reply message",
    'description': "You can change the default message which gaganrobot gives on un-invited PMs",
...
@gaganrobot.on_cmd("ipmmsg", about={
...
@gaganrobot.on_filters(~allowAllFilter & filters.incoming & filters.private & ~filters.bot
                   & ~filters.me & ~filters.service & ~Config.ALLOWED_CHATS, allow_via_bot=False)
async def uninvitedPmHandler(message: Message):
    """ pm message handler """
    user_dict = await gaganrobot.get_user_dict(message.from_user.id)
    ...
    if message.from_user.id in pmCounter:
        ...
        if gaganrobot.has_bot and _IS_INLINE:
            try:
                bot_username = (await gaganrobot.bot.get_me()).username
                k = await gaganrobot.get_inline_bot_results(bot_username, "pmpermit")
                await gaganrobot.send_inline_bot_result(
                    message.chat.id, query_id=k.query_id,
                    result_id=k.results[2].id, hide_via=True
                )
            except (IndexError, BotInlineDisabled):
                await message.reply(
                    noPmMessage.format_map(SafeDict(**user_dict)) + '\n`- Protected by gaganrobot`')
        else:
            await message.reply(
                noPmMessage.format_map(SafeDict(**user_dict)) + '\n`- Protected by gaganrobot`')
        await asyncio.sleep(1)
...
@gaganrobot.on_filters(~allowAllFilter & filters.outgoing & ~filters.edited
...
async def outgoing_auto_approve(message: Message):
    ...
    await ALLOWED_COLLECTION.update_one(
        {'_id': userID}, {"$set": {'status': 'allowed'}}, upsert=True)
    user_dict = await gaganrobot.get_user_dict(userID)
    await CHANNEL.log(f"**#AUTO_APPROVED**\n{user_dict['mention']}")

if gaganrobot.has_bot:
    @gaganrobot.bot.on_callback_query(filters.regex(pattern=r"pm_allow\((.+?)\)"))
    async def pm_callback_allow(_, c_q: CallbackQuery):
        owner = await gaganrobot.get_me()
        if c_q.from_user.id == owner.id:
            userID = int(c_q.matches[0].group(1))
            await gaganrobot.unblock_user(userID)
            user = await gaganrobot.get_users(userID)
            if userID in Config.ALLOWED_CHATS:
    ...
    @gaganrobot.bot.on_callback_query(filters.regex(pattern=r"pm_block\((.+?)\)"))
    async def pm_callback_block(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await gaganrobot.block_user(userID)
            if userID in pmCounter:
                del pmCounter[userID]
            if userID in Config.ALLOWED_CHATS:
                Config.ALLOWED_CHATS.remove(userID)
    ...
    @gaganrobot.bot.on_callback_query(filters.regex(pattern=r"^pm_spam$"))
    async def pm_spam_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            user_dict = await gaganrobot.get_user_dict(c_q.from_user.id)
            await c_q.edit_message_text(
                blocked_message.format_map(SafeDict(**user_dict)))
            await gaganrobot.block_user(c_q.from_user.id)
            await asyncio.sleep(1)
            await CHANNEL.log(
                f"#BLOCKED\n{c_q.from_user.mention} has been blocked due to spamming in pm !! ")

    @gaganrobot.bot.on_callback_query(filters.regex(pattern=r"^pm_contact$"))
    async def pm_contact_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await gaganrobot.bot.send_message(
                owner.id,
