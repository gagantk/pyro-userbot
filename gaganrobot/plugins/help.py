...
@gaganrobot.on_cmd("help", about={'header': "Guide to use GAGANROBOT commands"}, allow_channels=False)
async def helpme(message: Message) -> None:  # pylint: disable=missing-function-docstring
    plugins = gaganrobot.manager.enabled_plugins
...
if Config.BOT_TOKEN and Config.OWNER_ID:
    ...
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id == Config.OWNER_ID:
                ...
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your Own @GaganTK 🤘",
                    show_alert=True)
        return wrapper
    ...
    async def callback_next_prev(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 1:
            buttons = parse_buttons(p_num, cur_pos,
                                    ...
                                    gaganrobot.manager.get_all_plugins())
        elif len(pos_list) == 2:
            buttons = parse_buttons(p_num, cur_pos,
                                    lambda x: f"🗃 {x}",
                                    gaganrobot.manager.get_all_plugins()[pos_list[-1]])
        elif len(pos_list) == 3:
    ...
    async def callback_manage(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 4:
            if is_filter(pos_list[-1]):
                flt = gaganrobot.manager.filters[pos_list[-1]]
            else:
                flt = gaganrobot.manager.commands[pos_list[-1]]
            await getattr(flt, task)()
            text, buttons = filter_data(cur_pos)
        else:
            plg = gaganrobot.manager.plugins[pos_list[-1]]
    ...
    async def callback_mm(callback_query: CallbackQuery):
        await callback_query.edit_message_text(
            "🖥 **GaganRobot Main Menu** 🖥", reply_markup=InlineKeyboardMarkup(main_menu_buttons()))
    ...
    async def callback_chgclnt(callback_query: CallbackQuery):
        ...
        await SAVED_SETTINGS.update_one({'_id': 'CURRENT_CLIENT'},
                                        {"$set": {
                                            'is_user': Config.USE_USER_FOR_CLIENT_CHECKS}},
                                        upsert=True)
        await callback_query.edit_message_reply_markup(
    ...
    def category_data(cur_pos: str):
        pos_list = cur_pos.split('|')
        plugins = gaganrobot.manager.get_all_plugins()[pos_list[1]]
    ...
    def plugin_data(cur_pos: str, p_num: int = 0):
        pos_list = cur_pos.split('|')
        plg = gaganrobot.manager.plugins[pos_list[2]]
        ...
        results = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    "**Here's how to setup GaganRobot** 😎"
                ),
        ...
        if inline_query.from_user and inline_query.from_user.id == Config.OWNER_ID:
            results.append(
                InlineQueryResultArticle(
                    ...
                    title="Main Menu",
                    input_message_content=InputTextMessageContent(
                        "🖥 **GaganRobot Main Menu** 🖥"
                    ),
                    url="https://github.com/UsergeTeam/Userge",
                    description="GaganRobot Main Menu",
                    thumb_url="https://imgur.com/download/Inyeb1S",
                    reply_markup=InlineKeyboardMarkup(main_menu_buttons())
                )
            )
