...
from gaganrobot.utils import runcmd

CHANNEL = gaganrobot.getCLogger()


@gaganrobot.on_cmd("exec", about={
    'header': "run commands in exec",
    'usage': "{tr}exec [commands]",
    'examples': "{tr}exec echo \"GaganRobot\""}, allow_channels=False)
...
@gaganrobot.on_cmd("eval", about={
    ...
    'examples': [
        "{tr}eval print('GaganRobot')", "{tr}eval -s print('GaganRobot')",
...
async def eval_(message: Message):
    ...
    _g, _l = _context(
        context_type, gaganrobot=gaganrobot, message=message, replied=message.reply_to_message)
    l_d = {}
    try:
...
@gaganrobot.on_cmd("term", about={
    'header': "run commands in shell (terminal)",
    'usage': "{tr}term [commands]",
    'examples': "{tr}term echo \"GaganRobot\""}, allow_channels=False)
...
def _run_coro(future: asyncio.Future, coro: Awaitable[Any],
    ...
    gaganrobot.loop.call_soon_threadsafe(future.add_done_callback,
    ...
    try:
        ...
        loop.close()
        gaganrobot.loop.call_soon_threadsafe(lambda: future.done() or future.set_result(None))
