...
from gaganrobot.utils import get_import_path
from gaganrobot.plugins import ROOT


@gaganrobot.on_cmd("status", about={
...
async def status(message: Message) -> None:
    ...
    if not type_:
        out_str = f"""📊 **--GaganRobot Status--** 📊

🗃 **Plugins** : `{len(gaganrobot.manager.plugins)}`
        ✅ **Loaded** : `{len(gaganrobot.manager.loaded_plugins)}`
        ➕ **Enabled** : `{len(gaganrobot.manager.enabled_plugins)}`
        ➖ **Disabled** : `{len(gaganrobot.manager.disabled_plugins)}`
        ❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_plugins)}`

⚔ **Commands** : `{len(gaganrobot.manager.commands)}`
        ✅ **Loaded** : `{len(gaganrobot.manager.loaded_commands)}`
        ➕ **Enabled** : `{len(gaganrobot.manager.enabled_commands)}`
        ➖ **Disabled** : `{len(gaganrobot.manager.disabled_commands)}`
        ❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_commands)}`

⚖ **Filters** : `{len(gaganrobot.manager.filters)}`
        ✅ **Loaded** : `{len(gaganrobot.manager.loaded_filters)}`
        ➕ **Enabled** : `{len(gaganrobot.manager.enabled_filters)}`
        ➖ **Disabled** : `{len(gaganrobot.manager.disabled_filters)}`
        ❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_filters)}`
"""
    elif 'p' in type_:
        if name_:
            if name_ in gaganrobot.manager.plugins:
                plg = gaganrobot.manager.plugins[name_]
...
🗂 **Total** : `{len(gaganrobot.manager.plugins)}`
✅ **Loaded** : `{len(gaganrobot.manager.loaded_plugins)}`
➕ **Enabled** : `{len(gaganrobot.manager.enabled_plugins)}`
➖ **Disabled** : `{len(gaganrobot.manager.disabled_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in gaganrobot.manager.disabled_plugins))}`
❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in gaganrobot.manager.unloaded_plugins))}`
"""
    elif 'c' in type_:
        if name_:
            if not name_.startswith(Config.CMD_TRIGGER):
                n_name_ = Config.CMD_TRIGGER + name_
            if name_ in gaganrobot.manager.commands:
                cmd = gaganrobot.manager.commands[name_]
            elif n_name_ in gaganrobot.manager.commands:
                cmd = gaganrobot.manager.commands[n_name_]
            else:
...
🗂 **Total** : `{len(gaganrobot.manager.commands)}`
✅ **Loaded** : `{len(gaganrobot.manager.loaded_commands)}`
➕ **Enabled** : `{len(gaganrobot.manager.enabled_commands)}`
➖ **Disabled** : `{len(gaganrobot.manager.disabled_commands)}`
        `{'`,    `'.join((cmd.name for cmd in gaganrobot.manager.disabled_commands))}`
❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_commands)}`
        `{'`,    `'.join((cmd.name for cmd in gaganrobot.manager.unloaded_commands))}`
"""
    elif 'f' in type_:
        if name_:
            if name_ in gaganrobot.manager.filters:
                flt = gaganrobot.manager.filters[name_]
...
🗂 **Total** : `{len(gaganrobot.manager.filters)}`
✅ **Loaded** : `{len(gaganrobot.manager.loaded_filters)}`
➕ **Enabled** : `{len(gaganrobot.manager.enabled_filters)}`
        `{'`,    `'.join((flt.name for flt in gaganrobot.manager.enabled_filters))}`
➖ **Disabled** : `{len(gaganrobot.manager.disabled_filters)}`
        `{'`,    `'.join((flt.name for flt in gaganrobot.manager.disabled_filters))}`
❎ **Unloaded** : `{len(gaganrobot.manager.unloaded_filters)}`
        `{'`,    `'.join((flt.name for flt in gaganrobot.manager.unloaded_filters))}`
"""
    else:
...
@gaganrobot.on_cmd("enable", about={
...
async def enable(message: Message) -> None:
    ...
    type_ = list(message.flags)
    if 'p' in type_:
        found = set(names_).intersection(set(gaganrobot.manager.plugins))
        if found:
            out = await gaganrobot.manager.enable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.commands))
        if found:
            out = await gaganrobot.manager.enable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.filters))
        if found:
            out = await gaganrobot.manager.enable_filters(list(found))
            if out:
...
@gaganrobot.on_cmd("disable", about={
...
async def disable(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(gaganrobot.manager.plugins))
        if found:
            out = await gaganrobot.manager.disable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.commands))
        if found:
            out = await gaganrobot.manager.disable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.filters))
        if found:
            out = await gaganrobot.manager.disable_filters(list(found))
            if out:
...
async def load(message: Message) -> None:
    ...
    if message.flags:
        ...
        if 'p' in type_:
            found = set(names_).intersection(set(gaganrobot.manager.plugins))
            if found:
                out = await gaganrobot.manager.load_plugins(list(found))
                if out:
            ...
            found = set(names_).intersection(set(gaganrobot.manager.commands))
            if found:
                out = await gaganrobot.manager.load_commands(list(found))
                if out:
            ...
            found = set(names_).intersection(set(gaganrobot.manager.filters))
            if found:
                out = await gaganrobot.manager.load_filters(list(found))
                if out:
...
async def unload(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(gaganrobot.manager.plugins))
        if found:
            out = await gaganrobot.manager.unload_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.commands))
        if found:
            out = await gaganrobot.manager.unload_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(gaganrobot.manager.filters))
        if found:
            out = await gaganrobot.manager.unload_filters(list(found))
            if out:
...
@gaganrobot.on_cmd('reload', about={'header': "Reload all plugins"}, allow_channels=False)
async def reload_(message: Message) -> None:
    """ Reload all plugins """
    await message.edit("`Reloading All Plugins`")
    await message.edit(
        f"`Reloaded {await gaganrobot.reload_plugins()} Plugins`", del_in=3, log=__name__)


@gaganrobot.on_cmd('clear', about={'header': "clear all save filters in DB"}, allow_channels=False)
async def clear_(message: Message) -> None:
    """ clear all save filters in DB """
    await message.edit("`Clearing DB...`")
    await message.edit(
        f"**Cleared Filters** : `{await gaganrobot.manager.clear()}`", del_in=3, log=__name__)
