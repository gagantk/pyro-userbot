...
from gaganrobot import gaganrobot, Message, Config, logging, get_collection, pool
...
"""
    ...
    if thumb:
        args = {
            'caption': out_str,
            'parse_mode': "md",
            'reply_markup': markup if gaganrobot.has_bot else None
        }
    else:
        args = {
            'text': out_str,
    ...
    for chat_id in RSS_CHAT_ID:
        ...
        try:
            await send_rss_to_telegram(gaganrobot.bot, args, thumb)
        except (
            ChatWriteForbidden, ChannelPrivate, ChatIdInvalid,
            UserNotParticipant, GaganRobotBotNotFound
        ):
            ...
            await send_rss_to_telegram(gaganrobot, args, thumb)
...
@gaganrobot.on_cmd("addfeed", about={
...
@gaganrobot.on_cmd("delfeed", about={
...
@gaganrobot.on_cmd("listrss", about={
