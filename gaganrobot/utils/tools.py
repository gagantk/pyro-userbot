...
_LOG = gaganrobot.logging.getLogger(__name__)
...
def get_file_id_of_media(message: 'gaganrobot.Message') -> Optional[str]:
    """ get file_id """
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    ...
    ttl = duration // 2
    thumb_image_path = path or join(gaganrobot.Config.DOWN_PATH, f"{basename(video_file)}.jpg")
...
def is_command(cmd: str) -> bool:
    commands = gaganrobot.gaganrobot.manager.enabled_commands
    key = gaganrobot.Config.CMD_TRIGGER + cmd
    _key = gaganrobot.Config.SUDO_TRIGGER + cmd
