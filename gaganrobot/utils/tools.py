...
def get_file_id_and_ref(message: 'gaganrobot.Message') -> Tuple[Optional[str], Optional[str]]:
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    """ take a screenshot """
    _LOG.info('[[[Extracting a frame from %s ||| Video duration => %s]]]',
              video_file, duration)
    ttl = duration // 2
    thumb_image_path = path or os.path.join(
        gaganrobot.Config.DOWN_PATH, f"{basename(video_file)}.jpg")
