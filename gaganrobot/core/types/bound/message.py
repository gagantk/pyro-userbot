...
class Message(RawMessage):
    ...
    @property
    def client(self) -> Union['_client.GaganRobot', '_client._GaganRobotBot']:
        """ returns client """
        return self._client
    ...
    def input_or_reply_raw(self) -> str:
        ...
        if not input_ and self.reply_to_message:
            input_ = (
                self.reply_to_message.text.html if self.reply_to_message.text else self.reply_to_message.caption.html if self.reply_to_message.caption else '').strip()
        return input_
